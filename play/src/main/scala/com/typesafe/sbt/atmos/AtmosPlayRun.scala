...
object AtmosPlayRun {
  ...
  class RunHook(inputs: AtmosInputs, sigarLibs: Option[File], log: Logger) extends AtmosController(inputs) with play.PlayRunHook {
    ...
    override def afterStopped(): Unit = stop(log)
  }
}
